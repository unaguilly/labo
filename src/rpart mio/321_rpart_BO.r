...
ksemilla_azar  <- 609277   #cambiar por la primer semilla
...
ArbolesCrossValidation  <- function( data, param, qfolds, pagrupa, semilla )
{
  ...
  ganancias  <- mcmapply( ArbolSimple, 
                          ...
                          SIMPLIFY= FALSE,
                          mc.cores= qfolds )   #se puede subir a qfolds si posee Linux o Mac OS
...
#cargo el dataset
dataset  <- fread("./datasets/competencia1_2022_FE_2.csv")   #donde entreno
...
archivo_log  <- "HT321_pred.txt"
archivo_BO   <- "HT321_pred.RDATA"
