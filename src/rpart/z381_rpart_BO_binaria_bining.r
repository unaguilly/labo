...
ksemilla_azar  <- c(102191)  #Mas de una semilla cambia el  1-Repeated
...
particionar  <- function( pdata, division, agrupa="", campo="fold", start=1, seed=NA )
{
  if( !is.na( seed)  )   set.seed( seed )

  bloque  <- unlist( mapply(  function(x,y) { rep( y, x ) }, division, seq( from=start, length.out=length(division) )  ) )

  pdata[ , (campo) :=  sample( rep( bloque, ceiling(.N/length(bloque))) )[1:.N],
           by= agrupa ]
}
#------------------------------------------------------------------------------
...
ArbolSimple  <- function( fold_test, pdata, param )
{
  param2 <- param
  ...
  modelo  <- rpart("clase_binaria ~ . -clase_ternaria",
                   data= pdata[ fold != fold_test, ],  #entreno en todo MENOS el fold_test que uso para testing
                   xval= 0,
                   control= param2 )

  #aplico el modelo a los datos de testing
  prediccion  <- predict( modelo, 
                          pdata[ fold==fold_test, ],  #aplico el modelo sobre los datos de testing
  ...
  dtest <- copy( pdata[ fold==fold_test , list( clase_ternaria )] )
  ...
  limite_inf  <- pmax( 1, (idx_max-1) )
  limite_sup  <- pmin( nrow(dtest2), (idx_max+1) )
  ganancia_testing  <- dtest2[ limite_inf:limite_sup,  mean(gan_acum, na.rm=TRUE) ]
...
ArbolesCrossValidation  <- function( semilla, pdata, param, qfolds, pagrupa )
{
  divi  <- rep( 1, qfolds )  # generalmente  c(1, 1, 1, 1, 1 )  cinco unos

  particionar( pdata, divi, seed=semilla, agrupa=pagrupa )  #particiono en dataset en folds

  ganancias  <- mcmapply( ArbolSimple, 
                          seq(qfolds), # 1 2 3 4 5
                          MoreArgs= list( pdata, param), 
                          SIMPLIFY= FALSE,
...
dir.create( "./exp/HT3711/", showWarnings = FALSE )
setwd("./exp/HT3711/")   #Establezco el Working Directory DEL EXPERIMENTO

#defino los archivos donde guardo los resultados de la Bayesian Optimization
archivo_log  <- "HT3710.txt"
archivo_BO   <- "HT3710.RDATA"
